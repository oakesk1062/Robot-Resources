...
class color_identify:
    ...
    def select_color(self, image, color_hsv, color_list):
        ...
        # Get information about the second color sequence
        # 获取第二个颜色序列的信息
        if len(color_list) > 1:
            name_pos = self.color_(color_hsv, color_list[1])
            if name_pos != None: msg.append(name_pos)
        # Get information on the third color sequence
        # 获取第三个颜色序列的信息
        if len(color_list) > 2:
            name_pos = self.color_(color_hsv, color_list[2])
            if name_pos != None: msg.append(name_pos)
        # Get information on the fourth color sequence
        # 获取第四个颜色序列的信息
        if len(color_list) > 3:
